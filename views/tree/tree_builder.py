                ...
                for idx, col in enumerate(budget_cols):
                    try:
                        ...
                        if level < 6 and self._is_value_different(original_approved, calculated_approved):
                            ...
                            col_idx = approved_start + idx
                            if col_idx < column_count:
                                tree_item.setText(col_idx, approved_value)
                                tree_item.setForeground(col_idx, QBrush(error_color))
                        else:
                        ...
                        if level < 6 and self._is_value_different(original_executed, calculated_executed):
                            ...
                            col_idx = executed_start + idx
                            if col_idx < column_count:
                                tree_item.setText(col_idx, executed_value)
                                tree_item.setForeground(col_idx, QBrush(error_color))
                        else:
                ...
                for idx, col in enumerate(cons_cols):
                    try:
                        ...
                        if should_check and self._is_value_different(original_value, calculated_value):
                            ...
                            col_idx = value_start + idx
                            if col_idx < column_count:
                                tree_item.setText(col_idx, display_value)
                                tree_item.setForeground(col_idx, QBrush(error_color))
                        else:
                            col_idx = value_start + idx
                            if col_idx < column_count:
                                tree_item.setText(col_idx, self.format_budget_value(original_value))
                    except Exception as e:
