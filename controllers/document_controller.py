...
class DocumentController(QObject):
    ...
    def _extract_form_data(self, revision_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        результат_исполнения_data = revision_data.get('результат_исполнения_data', {})
        ...
        # Извлекаем значения для дефицита/профицита
        утвержденный_дефицит = результат_исполнения_data.get('утвержденный', {})
        исполненный_дефицит = результат_исполнения_data.get('исполненный', {})
        ...
        return {
            ...
            'дефицит_утвержденный': утвержденный_дефицит.get(budget_col, 0),
            'дефицит_исполненный': исполненный_дефицит.get(budget_col, 0),
            'slojilsya': slojilsya,
