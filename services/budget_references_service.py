...
class BudgetReferencesService:
    ...
    # def fill_all_references(
    #     self, 
    #     filters: Optional[Dict[str, Dict]] = None,
    #     clear_existing: bool = True,
    #     default_region: str = "21"
    # ) -> Dict[str, int]:
    #     """
    #     Заполняет все справочники из API.
        
    #     Args:
    #         filters: Словарь {table_name: filters_dict} для фильтрации данных.
    #                 Если не указан, используются фильтры по умолчанию.
    #         clear_existing: Очищать ли существующие данные перед загрузкой
    #         default_region: Код региона по умолчанию (для фильтрации МО справочников)
            
    #     Returns:
    #         Словарь {table_name: count} с количеством вставленных записей
    #     """
    #     default_filters = get_default_filters(default_region)
        
    #     # Используем переданные фильтры или фильтры по умолчанию
    #     if filters is None:
    #         filters = default_filters
    #     else:
    #         # Объединяем переданные фильтры с фильтрами по умолчанию
    #         for table_name, default_filter in default_filters.items():
    #             if table_name not in filters:
    #                 filters[table_name] = default_filter
        
    #     tasks = [
    #         (URL_OKTMO, 'oktmo', filters.get('oktmo'), clear_existing),
    #         (URL_BUDGETCLASTYPEINC, 'budgetclastypeinc', filters.get('budgetclastypeinc'), clear_existing),
    #         (URL_BUDGETCLASSUBTYPINCMO, 'budgetclassubtypincmo', filters.get('budgetclassubtypincmo'), clear_existing),
    #         (URL_BUDGETCLASGABS, 'budgetclasgabs', filters.get('budgetclasgabs'), clear_existing),
    #         (URL_BUDGETCLASGABSMO, 'budgetclasgabsmo', filters.get('budgetclasgabsmo'), clear_existing),
    #         (URL_BUDGETCLASGRBS, 'budgetclasgrbs', filters.get('budgetclasgrbs'), clear_existing),
    #         (URL_BUDGETCLASGRBSMO, 'budgetclasgrbsmo', filters.get('budgetclasgrbsmo'), clear_existing),
    #         (URL_BUDGETCLASCOSTS, 'budgetclascosts', filters.get('budgetclascosts'), clear_existing),
    #         (URL_BUDGETCLASCOSTSMO, 'budgetclascostsmo', filters.get('budgetclascostsmo'), clear_existing),
    #         (URL_BUDGETCLASGAIFFB, 'budgetclasgaiffb', filters.get('budgetclasgaiffb'), clear_existing),
    #         (URL_BUDGETCLASGAIFMO, 'budgetclasgaifmo', filters.get('budgetclasgaifmo'), clear_existing),
    #         (URL_BUDGETCLASSOURCES, 'budgetclassources', filters.get('budgetclassources'), clear_existing),
    #         (URL_BUDGETCLASSOURCESMO, 'budgetclassourcesmo', filters.get('budgetclassourcesmo'), clear_existing),
    #     ]
        
    #     results = {}
    #     total_inserted = 0
        
    #     for url, table_name, table_filters, clear in tasks:
    #         inserted = self.fill_table_from_url(url, table_name, table_filters, clear)
    #         results[table_name] = inserted
    #         total_inserted += inserted
        
    #     logger.info(f"\n{'='*60}")
    #     logger.info(f"Заполнение завершено. Всего вставлено записей: {total_inserted}")
